...
function library:New(name) 
	...
	cursor.Name = "cursor" 
	cursor.Parent = lucky 
	cursor.BackgroundTransparency = 01
	...
	function menu:Tab(text) 
		...
		TextButton.MouseButton1Down:Connect(function() 
			if currenttab ~= text then 
				...
				library:Tween(TextButton, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(0, 220, 0)}) 
				currenttab = text 
				TabGui.Visible = true 
			end 
		end) 
		...
		function Tab:MSector(text, side) 
			...
			function MSector:Tab(text) 
				...
				TextButton.MouseButton1Down:Connect(function() 
					...
					tab1.Visible = true 
					library:Tween(TextButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(0, 220, 0)}) 
				end) 
				...
				function tab:Element(type, text, data, callback) 
							...
							do 
								...
								Button.MouseButton1Down:Connect(function() 
									if TBLFIND(Element.value.Jumbobox, v) then 
										...
										library:Tween(TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(0, 220, 0)}) 
									end 
									updatetext() 
					...
					elseif type == "ToggleKeybind" then 
						...
						local function update() 
							if Element.value.Toggle then 
								tween = library:Tween(Color, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = COL3RGB(0, 220, 0)}) 
								library:Tween(TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 255)}) 
							else 
								keybindremove(text) 
								...
								if button:IsA("TextButton") then 
									...
									button.MouseEnter:Connect(function() 
										if Element.value.Type ~= button.Text then 
											library:Tween(button, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(0, 220, 0)}) 
										end 
									end) 
									button.MouseLeave:Connect(function() 
										if Element.value.Type ~= button.Text then 
					...
					elseif type == "Button" then 
						...
						Button_2.MouseEnter:Connect(function() 
							library:Tween(TextLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(0, 220, 0)}) 
						end) 
			...
			function Sector:Element(type, text, data, callback) 
				...
				elseif type == "Scroll" then      
					...
					function Element:SetValue(val)      
						...
						library:Tween(Frame[Element.value.Scroll].TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(0, 220, 0)})      
						values[tabname][sectorname][text] = Element.value      
						callback(Element.value)      
					end      
					values[tabname][sectorname][text] = Element.value      
...
local gui = library:New("Lucky.cc") 
...
luascripts:Element("Button", "load", {}, function() 
	loadstring(readfile("lucky\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 
luascripts:Element("Button", "Lucky", nil, function()
...
aimbot:Element("Jumbobox", "resolver", {options = {"pitch", "roll", "animation", "lucky"}}) 
...
aimbot:Element("Dropdown", "autowall type", {options = {"buggy", "standard"}}) 
...
local function AddRage(Tab) 
	...
	Tab:Element("Slider", "minimum damage", {min = 0, max = 100, default = 1})
...
antiaim:Element("Dropdown", "pitch", {options = {"up", "down", "imposter", "random", "lucky", "custom", "huge"}})
...
others:Element("Dropdown", "slide walk", {options = {"off", "slide"}}) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Winning" and "Lucky.cc winning$$$$$" or "Join Lucky.cc https://discord.gg/34PSmnuKvm", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"Winning", "Lucky.cc$$$"}}) 
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
	...
	if values.misc.chat["kill say"].Toggle then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "Eating Lucky Charms" and values.misc.chat["message"].Text or "Lucky.cc on top", false, "Innocent", false, true) 
	end 
end) 
